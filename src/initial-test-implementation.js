...
describe("Scoreboard", () => {
  ...
  test("displays the summary of games in progress ordered by total score", () => {
    ...
    // Verify that the summary list is displayed and the games are ordered by total score
    expect(summaryList).toBeInTheDocument();
    const summaryItems = screen.getAllByTestId("summary-item");
    expect(summaryItems).toHaveLength(3);

    // Verify the order of the games based on the total score
    expect(summaryItems[0].textContent).toContain("Spain 3 - 2");
    expect(summaryItems[1].textContent).toContain("Mexico 1 - 0");
    expect(summaryItems[2].textContent).toContain("Uruguay 2 - 2");
  });
});
